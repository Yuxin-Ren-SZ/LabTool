...
| 5 legacy 清理 | ✅ 完成 | **已删除**：workbench 内联回退路径（legacyOpenDB/legacyDbExec/legacyCollectDescending）——store.js 全面接入（hub + 10 工具，types→store→workbench），存储单一走 labtools.store，缺 store 显式抛错（17c7a8f）；runtime SCRIPT_ORDER 修正（六必载依赖）。**文档收尾**（c67f3f9）：README/AGENTS/CONTRIBUTING/issue 模板 11 工具清单、CLAUDE.md 加载顺序/集成状态/新增工具流程。**冻结兼容层（后续删除条件见下）**：legacy serializeForWorkbench/applyFromWorkbench 与 typed Save/Load 按钮、DATA_TYPES 业务 type 注册、typed labtoolsHandoffTo/ConsumeHandoff 保留——它们仍驱动 typed 保存/加载按钮与 e2e 契约测试（live-validation/transforms）；删除条件 = ①typed Save/Load 按钮全部改 artifact 保存/加载（涉及全部工具的 UX 与测试迁移，规模相当于一个批次）②live-validation 改为 readOutputs→contracts 校验后。unit 170 + e2e 54 全绿 |
